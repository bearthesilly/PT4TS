...
class SparseTopologyGenerator:
    """
    Synthetic dataset for the Sparse Channel Topology prior.

    Graph: 8 channels on a chain:  0-1-2-3-4-5-6-7

    Key design: each edge (i, i+1) has a shared latent process.
    Each channel's signal = average of its edge-latents + HEAVY noise.
    ...
    The individual noise is large (SNR << 1 per single channel).

    To predict ch_c, the model needs to denoise edge-latents by looking at
    NEIGHBORS (who share those same edge-latents).  Non-neighbors share no
    edge-latents and are pure distractors.

    A model that knows the chain topology focuses only on adjacent channels
    → efficient denoising → better prediction.
    """
    ...
    def generate(self, save_path='./dataset/syn_data/sparse_topo_150.npy'):
        print(f"Generating {self.num_samples} sparse-topology samples "
...
if __name__ == "__main__":
    gen = SparseTopologyGenerator(num_samples=150, sample_len=192, seed=2025)
    gen.generate(save_path='./dataset/syn_data/sparse_topo_150.npy')
