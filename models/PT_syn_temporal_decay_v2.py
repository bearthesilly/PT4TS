'''
ST-PT + Temporal Decay Prior v2 (Decay-Weighted Unary + Multiplicative F/G Modulation)

This model is based on PT_forecast_v15.py (the vanilla ST-PT model).
The differences are:
1. PtHeadSelection: adds temporal decay prior to calculate_messageF and calculate_messageG
   for the time dimension, modulating attention by exp(-alpha * |i - j|).
2. PtModel: applies recency weighting to unary potentials (latest patch = 1.0).
...
class PtHeadSelection(nn.Module):
    ...
    Modified for Temporal Decay Prior: adds exp-decay modulation to time-axis
    message_F and message_G computations.
    """
    ...
    def _build_decay_prior(self):
        """Build [L, L] multiplicative exp-decay prior: exp(-alpha * |i - j|)."""
        idx = torch.arange(self.patch_num, dtype=torch.float32)
        dist = torch.abs(idx.unsqueeze(1) - idx.unsqueeze(0))
        return torch.exp(-self.decay_alpha * dist)
    ...
    def calculate_messageF(self, qz, dependency_mask, position_ids, position_embeddings, ternary_factor_u, ternary_factor_v, dimension=None):
        ...
        # Temporal decay prior: multiplicative modulation for time dimension
        if dimension == 'time':
            decay = self.decay_prior.to(device=message_F.device, dtype=message_F.dtype)
            message_F = message_F * decay
    ...
    def calculate_messageG(self, qh, qz_uo, qz_v, bsz, seq_len, position_ids, position_embeddings, ternary_factor_u, ternary_factor_v, dimension=None):
        ...
        # Temporal decay prior: modulate qh before matmul for time dimension
        if dimension == 'time':
            decay = self.decay_prior.to(device=qh.device, dtype=qh.dtype)
            qh = qh * decay
...
class PtModel(nn.Module):
    ...
    def forward(
        ...
        # Temporal decay prior: weight unary potentials by recency
        # latest patch = 1.0, oldest patch = exp(-alpha * (patch_num - 1))
        recency = torch.exp(-self.decay_alpha * torch.arange(
            self.patch_num - 1, -1, -1, device=device, dtype=unary_potentials.dtype
        ))  # [patch_num], latest=1.0, oldest=exp(-alpha*(P-1))
        unary_potentials = unary_potentials * recency.view(1, 1, self.patch_num, 1)
