'''
ST-PT + Sparse Topology Prior v2 (Topology Message Passing)

This model is based on PT_forecast_v15.py (the vanilla ST-PT model).
The only difference is the addition of PtTopologyMessage in PtEncoderIterator,
which passes messages along known graph edges for sparse topology priors.
...
class PtEncoderIterator(nn.Module):
    def __init__(self, args):
        ...
        self.topology_message = PtTopologyMessage(args)
    ...
    def forward(
        ...
        old_qz = qz
        qz = self.norm(qz)
        # Sparse topology prior: neighbor-aggregation message along known edges
        ...
        # m_topo: message from sparse topology prior
        qz = (m_t + m_c + m_g + m_topo + unary_potentials) / self.config.regularize_z
        # damping
        qz = (qz + old_qz) * .5
