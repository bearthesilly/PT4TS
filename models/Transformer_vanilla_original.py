...
class Encoder(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, dropout, activation, num_layers, norm_layer=None):
        ...
        self.layers = nn.ModuleList([
            EncoderLayer(d_model, n_heads, d_ff, dropout, activation) for _ in range(num_layers)
        ])
    ...
    def forward(self, x, attn_mask=None, embedding=None):
        ...
        for layer in self.layers:
            x, attn = layer(x, attn_mask, embedding)
            attns.append(attn)
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec=None, x_mark_dec=None, mask=None):
        ...
        if self.task_name in ['long_term_forecast', 'short_term_forecast']:
            dec_out = self.decoder1(enc_out).transpose(1, 2)
            dec_out = self.decoder2(dec_out).transpose(1, 2)
            dec_out = dec_out * \
