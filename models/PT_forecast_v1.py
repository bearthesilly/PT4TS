...
Compared to v4, this model has no implicitHMM enhancement
...
This is considered as the vanilla model
'''
import math
import warnings
...
class PtHeadSelection(nn.Module):
    ...
    def show_head_train(self):
        ...
        qh_mean = self.qh.mean(dim=0)[0].cpu().numpy()  # shape: (num_channels, seq_len, seq_len)
        F_mean = self.original_F.mean(dim=0)[0].cpu().numpy()  # shape: (num_channels, seq_len, seq_len)
        # Create a heatmap
        plt.figure(figsize=(10, 8))
        sns.heatmap(qh_mean, cmap="viridis", cbar=True, square=True, annot=False, fmt=".2f")
        plt.savefig("./attn_vis/vanilla_qh_train_seed803.png")
        # Create the heatmap for original_F as well
        plt.figure(figsize=(10, 8))
        sns.heatmap(F_mean, cmap="viridis", cbar=True, square=True, annot=False, fmt=".2f")
        plt.savefig("./attn_vis/vanilla_F_train_seed803.png")
...
class PtModel(nn.Module):
    ...
    def forward(
        ...
        dec_out = self.predict_2(qz.transpose(1, 2)).transpose(1, 2)  # [bs, pred_len, c_out]
        dec_out = self.predict_1(dec_out)  # [bs, length, dim_z]
        # dec_out = self.predict_1(dec_out)  # [bs, pred_len, c_out]
        dec_out = dec_out * \
