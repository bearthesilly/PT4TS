'''
ST-PT + Channel Grouping Prior v2 (Group Pooling Message)

This model is based on PT_forecast_v15.py (the vanilla ST-PT model).
The only difference is the addition of PtGroupPooling in PtEncoderIterator,
which computes within-group consensus messages for channel grouping.
...
class PtEncoderIterator(nn.Module):
    def __init__(self, args):
        ...
        self.group_pooling = PtGroupPooling(args)
    ...
    def forward(
        ...
        old_qz = qz
        qz = self.norm(qz)
        # Channel grouping prior: within-group consensus message
        m_group = self.group_pooling(qz)
        # head selection
        ...
        # m_group: message from channel grouping prior
        qz = (m_t + m_c + m_g + m_group + unary_potentials) / self.config.regularize_z
        # damping
        qz = (qz + old_qz) * .5
