...
Note that the above are MLPs, not linear transformations
This model has period enhancement with the right period
The second approach of the period variant
'''
import math
import warnings
...
class PtHeadSelection(nn.Module):
    ...
    def forward(
        ...
        if dependency_mask is not None:
            ...
            message_F = message_F + dependency_mask # [bs, num_head, len, len]
        self.original_F = message_F
        # Adding period mask to message F
        x = torch.arange(seq_len).view(-1, 1)
        y = torch.arange(seq_len).view(1, -1)
        matrix = (torch.abs(x - y) == self.config.period).float()*self.config.regularize_h*20
        message_F[:, 0, :, :] += matrix.to(message_F.device) # only apply to the first head
    ...
    def show_head_train(self):
        # Print the heatmap, i.e., self.qh, but we take the mean of a batch for visualization
        if self.qh is None:
            raise RuntimeError("No attention weights computed yet. Run the forward pass first.")
        import matplotlib.pyplot as plt
        import seaborn as sns
        import numpy as np
        # preserve the heat map of self.original_F and self.qh
        # Take the mean across the batch dimension
        print('Preserving the mean of the attention weights across the batch dimension.')
        for i in range(config.num_channels):
            qh_mean = self.qh.mean(dim=0)[i].cpu().numpy()  # shape: (num_channels, seq_len, seq_len)
            F_mean = self.original_F.mean(dim=0)[i].cpu().numpy()  # shape: (num_channels, seq_len, seq_len)
            # Create a heatmap
            plt.figure(figsize=(10, 8))
            sns.heatmap(qh_mean, cmap="viridis", cbar=True, square=True, annot=False, fmt=".2f")
            string = "./attn_vis_new/navilla_qh_train_seed803_" + str(i) + "_.png"
            plt.savefig(string)
            # Create the heatmap for original_F as well
            plt.figure(figsize=(10, 8))
            sns.heatmap(F_mean, cmap="viridis", cbar=True, square=True, annot=False, fmt=".2f")
            string = "./attn_vis_new/vanilla_F_train_seed803_" + str(i) + "_.png"
            plt.savefig(string)
...
class PtModel(nn.Module):
    ...
    def forward(
        ...
        # print(means.shape)
        # print(stdev.shape)
        # print(input_ids.mean(1, keepdim=True))
        # print(torch.var(input_ids, dim=1, keepdim=True, unbiased=False))
        # raise RuntimeError()
