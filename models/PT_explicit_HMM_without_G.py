...
class PtExplicitHMM(nn.Module):
    def __init__(self, config, args):
        ...
        self.binary_potential_table = nn.Parameter(torch.empty(self.dim_z, self.dim_m)) # between Z and M, Z->M
        self.markovian_potential = nn.Parameter(torch.empty(self.dim_m, self.dim_m)) # between M and M
        self.act = POTENTIAL2ACT[config.potential_func_g](dim=-1, eps=config.potential_eps)
    ...
    def forward(self, qz, qm):
        if (qm == None):
            qm = nn.functional.linear(qz, self.binary_potential_table.T) * self.config.binary_table_scaling
        ...
        front2back[:, 1:, :] = f2b[:, :-1, :]
        front2back[:, 0, :] = 0
        z2markov = nn.functional.linear(qz, self.binary_potential_table.T) * self.config.binary_table_scaling
        ...
        qm = self.act(qm / self.config.regularize_g)
        markov2z = nn.functional.linear(qm, self.binary_potential_table) * self.config.binary_table_scaling
        return markov2z, qm
...
class PtEncoderIterator(nn.Module):
    ...
    def forward(
        ...
        # unary potentials
        qz = (m1 + unary_potentials + markov2z) / self.config.regularize_z
...
class Model(nn.Module):
    ...
    def forward(
        ...
        """
        config.length = input_ids.shape[1]
        config.batch_size = input_ids[0]
        outputs = self.model(
            input_ids,
