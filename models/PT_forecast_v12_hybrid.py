...
class PtHeadSelection(nn.Module):
    ...
    def forward(
        ...
        matrix = (torch.abs(x - y) == 24).float()*self.dim_z*self.args.period_scale
        message_F[:, 0, :, :] += matrix.to(message_F.device) # only apply to the first head
        qh = nn.functional.softmax(message_F * self.dim_z, dim=-1, dtype=torch.float32).to(qz_u.dtype)
...
class PtImplicitHMM(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.dim_z = args.d_model
        self.binary_potential_table = nn.Parameter(torch.empty(self.dim_z, self.dim_z))
        self.identity = torch.eye(self.dim_z).to(self.binary_potential_table.device)
        self._init_ternary()
    
    def _init_ternary(self):
        nn.init.normal_(self.binary_potential_table, mean=0.0, std=0.2)
    def forward(self, qz):
        self.identity = self.identity.to(qz.device)
        back2front = nn.functional.linear(qz, (self.binary_potential_table+self.identity).T) * self.args.trend_scale
        front2back = nn.functional.linear(qz, (self.binary_potential_table+self.identity)) * self.args.trend_scale
...
class PtEncoderIterator(nn.Module):
    def __init__(self, args):
        ...
        self.implicit_hmm = PtImplicitHMM(args)
    ...
    def forward(
        ...
        b2f, f2b = self.implicit_hmm(qz)
        # topic modeling
        m2 = self.topic_modeling(qz)
        # unary potentials
        qz = (m1 + m2 + unary_potentials + b2f + f2b) * self.dim_z
        # damping
        qz = (qz + old_qz) * .5
        outputs = (qz,)
