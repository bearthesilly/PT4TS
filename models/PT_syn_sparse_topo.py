'''
ST-PT + Sparse Channel Topology Prior  (Edge Removal on channel axis)

Based on PT_forecast_v15 (backbone).
Prior: Given a known sparse adjacency graph over channels (e.g. a chain),
we remove ternary factor edges between non-adjacent channels by adding
-inf to their entries in the channel dependency mask.

Graph primitive:  Edge Removal
