...
class PtHeadSelection(nn.Module):
    ...
    def __init__(self, args):
        ...
        self._init_ternary()
        # [enc_in, patch_num, patch_num]
        self.period_prior = self.period_bias_matrix()
    ...
    def period_bias_matrix(self):
        ...
        bias_block = torch.stack(bias_matrix_list, dim=0) * 5.0 # remember to use float
        return bias_block
    ...
    def calculate_messageF(self, qz, dependency_mask, position_ids, position_embeddings, ternary_factor_u, ternary_factor_v, dimension: str=None):
        ...
        if dimension == 'time':
            ...
            prior_expanded = self.period_prior.unsqueeze(0).repeat(batch_size, 1, 1, 1).view(-1, self.patch_num, self.patch_num).to(message_F.device)
            # [batchsize*enc_in, num_channels(num_heads), patch_num, patch_num]
            message_F[:, 0, :, :] += prior_expanded.to(message_F.dtype)
