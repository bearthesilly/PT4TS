...
class PtModel(nn.Module):
    def __init__(self, args):
        ...
        self.seq_len = args.enc_in # This is the number of variables
        ...
        self.unary_factors = nn.Linear(args.seq_len, self.dim_z)

        # +++ 可视化相关属性 +++
        self.visualize = getattr(args, 'visualize', False)
        self.visualize_output_dir = getattr(args, 'output_path', './output')
        if self.visualize:
    ...
    def forward(
        ...
        # In this iTransformer-like setting, the number of variables is the sequence length
        num_variables = input_ids.shape[1]
        dependency_mask = torch.ones((input_ids.shape[0], num_variables), device = device)
        ...
        for idx in range(self.num_iteration):
            ...
            # For visualization, we only care about the dependencies in the last iteration
            is_last_iter = (idx == self.num_iteration - 1)
            ...
            iter_outputs = self.iterator(
                ...
                dependency_mask=dependency_mask,
                position_ids=position_ids,
                output_dependencies=output_dependencies and is_last_iter,
                position_embeddings=position_embeddings,
            )
            qz = iter_outputs[0]
            if output_dependencies and is_last_iter:
        ...
        dec_out = self.prediction(qz).permute(0, 2, 1)
        dec_out = dec_out * \
        ...
        # +++ 如果是可视化模式，则返回包含依赖矩阵的元组 +++
        if self.visualize and all_qhs is not None and len(all_qhs) > 0:
            return dec_out, all_qhs[0]
        # --- 如果是可视化模式，则返回包含依赖矩阵的元组 ---
    ...
    # +++ 新增的可视化方法 +++
    def visualize_and_save_dependencies(self, qh, variable_names=None, batch_idx=0):
        """
        Visualizes the pairwise dependency matrix (qh) and saves it as a heatmap.

        Args:
            qh (torch.Tensor): The dependency tensor of shape [bs, num_heads, num_vars, num_vars].
            variable_names (list of str, optional): Names of the variables for labeling the axes.
            batch_idx (int): The index of the batch item to visualize.
        """
        if qh is None:
            print("No dependency matrix (qh) to visualize.")
            return

        # Detach from graph and move to CPU
        qh_to_plot = qh.detach().cpu()

        # Average over heads
        avg_qh = qh_to_plot.mean(dim=1) # Shape: [bs, num_vars, num_vars]

        # Select a specific item from the batch
        dependency_matrix = avg_qh[batch_idx] # Shape: [num_vars, num_vars]
        
        num_vars = dependency_matrix.shape[0]
        ...
        ax = sns.heatmap(
            ...
            annot=False, # Annotations can be cluttered for large matrices
            cmap='viridis'
        )
        plt.title(f'Learned Variable Dependencies (Batch Item {batch_idx})')
        plt.xlabel('Information Source Variable')
        plt.ylabel('Information Target Variable')
        
        save_path = os.path.join(self.visualize_output_dir, f'dependency_heatmap_batch_{batch_idx}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Saved dependency heatmap to {save_path}")
    # --- 新增的可视化方法 ---
...
class Model(nn.Module):
    ...
    def forward(
        ...
        # +++ 处理可视化逻辑 +++
        if self.model.visualize and isinstance(outputs, tuple):
            dec_out, qh = outputs
            self.model.visualize_and_save_dependencies(
                qh, 
