...
class PtImplicitHMM(nn.Module):
    ...
    def forward(self, qz):
        ...
        back2front = nn.functional.linear(qz, (self.binary_potential_table+self.identity).T) * self.config.binary_table_scaling
        front2back = nn.functional.linear(qz, (self.binary_potential_table+self.identity)) * self.config.binary_table_scaling
...
class PtEncoderIterator(nn.Module):
    def __init__(self, config):
        ...
        self.implicit_hmm = PtImplicitHMM(config)
    ...
    def forward(
        ...
        # unary potentials
        qz = (m1 + m2 + unary_potentials + b2f + f2b) / self.config.regularize_z
...
class PtModel(nn.Module):
    def __init__(self, args):
        ...
        self.projection = nn.Linear(self.dim_z * args.seq_len, args.num_class)
    ...
    def forward(
        ...
        # If dropout = 0.1 is needed
        # qz = self.dropout(qz)
        
        
        output = qz.reshape(qz.shape[0], -1) # (batch_size, seq_length * d_model)
        logit = self.projection(output)
