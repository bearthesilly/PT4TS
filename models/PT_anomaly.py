...
class PtModel(nn.Module):
    ...
    def forward(
        ...
        dependency_mask = torch.ones((input_ids.shape[0], self.pred_len + self.seq_len), device = device)
        # normalization
        ...
        x_enc /= stdev
        # encode into dim_z for the PT
        unary_potentials = self.unary_factors(x_enc, None) # [bs, seq, dim_z]
        ...
        # create the real mask. Note that the shape of mask and unary_potentials should be the same!
