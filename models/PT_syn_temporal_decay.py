'''
ST-PT + Temporal Locality / Decay Prior  (Potential Modulation on time axis)

Based on PT_forecast_v15 (backbone).
Prior: Multiply an exponential decay bias  exp(-alpha * |i - j|)  onto the
temporal ternary factor messages (messageF along time), so that nearby
patches contribute more strongly.  This encodes the "short memory" prior
characteristic of AR(k) processes.

Graph primitive:  Potential Modulation
Injection point:  messageF_time and qh_time  (multiplicative bias)
'''
import math
import warnings
...
class PtHeadSelection(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Temporal decay prior: need patch geometry
