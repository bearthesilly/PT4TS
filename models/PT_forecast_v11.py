...
In this model, we use positional embedding instead of rotary embedding
'''
import math
import warnings
...
class Config:
    ...
    regularize_h: float = 0.001953125
    regularize_g: float = 1.0
    hidden_size: int = 768
    hidden_act: str = "silu"
    layer_norm_eps: float = 1e-05
...
class PtModel(nn.Module):
    def __init__(self, args):
        ...
        self.position_embedding = PositionalEmbedding(d_model=self.dim_z, max_len=self.seq_len)
    ...
    def forward(
        ...
        unary_potentials += self.position_embedding(input_ids)
